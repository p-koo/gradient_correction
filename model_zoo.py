...
def deepatt(input_shape, num_labels, activation='relu', batch_size=100):
    ...
    nn,_ = MultiHeadAttention(400, 4)(query, nn, nn)
    nn = keras.layers.Dropout(0.2)(nn)
    print(nn)
...
def cnn_att(input_shape, num_labels, activation='relu'):
    ...
    # layer 3
    nn,_ = MultiHeadAttention(512, 8)(nn, nn, nn)
    nn = keras.layers.Dropout(0.2)(nn)
...
def cnn_lstm_trans(input_shape, num_labels, activation='relu', num_layers=1):
    ...
    for i in range(num_layers):
        nn2,_ = MultiHeadAttention(d_model=512, num_heads=8)(nn, nn, nn)
...
def cnn_trans(input_shape, num_labels, activation='relu', num_layers=1):
    ...
    nn = keras.layers.Conv1D(filters=key_size, kernel_size=7, use_bias=False, padding='same')(inputs)
    nn = keras.layers.BatchNormalization()(nn)
    nn = keras.layers.Activation(activation)(nn)
...
def deepsea_custom(input_shape, num_labels, activation='relu'):
    ...
    # layer 1
    nn = keras.layers.Conv1D(filters=320, kernel_size=8, use_bias=True, padding='same')(inputs)
    nn = keras.layers.Activation(activation)(nn)
    nn = keras.layers.MaxPool1D(pool_size=4)(nn)
    nn = keras.layers.Dropout(0.2)(nn)

    # layer 2
    nn = keras.layers.Conv1D(filters=480, kernel_size=8, use_bias=False, padding='same', activation=None)(nn)
    nn = keras.layers.BatchNormalization()(nn)      
    nn = keras.layers.Activation(activation)(nn)
    nn = keras.layers.MaxPool1D(pool_size=4)(nn)
    nn = keras.layers.Dropout(0.2)(nn)
    ...
    # layer 3
    nn = keras.layers.Conv1D(filters=960, kernel_size=8, use_bias=False, padding='same', activation=None)(nn)
    nn = keras.layers.BatchNormalization()(nn)        
    nn = keras.layers.Activation(activation)(nn)
    nn = keras.layers.MaxPool1D(pool_size=4)(nn)
    nn = keras.layers.Dropout(0.5)(nn)
    ...
    # layer 4 - Fully-connected 
    nn = keras.layers.Dense(925, activation=None, use_bias=False)(nn) 
    nn = keras.layers.BatchNormalization()(nn)        
    nn = keras.layers.Activation(activation)(nn)
    nn = keras.layers.Dropout(0.5)(nn)  
